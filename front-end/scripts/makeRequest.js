const categoryToSubcategories = {
  ...
  const areCheckboxesValid = (checkInBox.checked && !checkOutBox.checked) || (!checkIn.checked && checkOut.checked);
  const areSelectsValid = [...document.getElementById('subcategories').getElementsByTagName('select')].every(select => select.value !== '');
  const areCategoriesValid = categoryElement.value != '';

  const isValid = areSelectsValid && areCheckboxesValid && areCategoriesValid;
  ...
  function handleSubmit(event) {
    event.preventDefault(); // This line prevents the form from resetting before modals
    reqItem('req');
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    checkInBox.addEventListener('change', () => {
      validateForm();
    });
